...
class InfiniteGridMenu {
  ...
  constructor(canvas, items, onActiveItemChange, onMovementChange, onInit = null, scale = 1.0, onPositionChange = null) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  #initTexture() {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        if (item?.brightness) ctx.filter = `brightness(${item.brightness})`;
  ...
  #animate(deltaTime) {
    ...
    // Track nearest vertex every frame so title follows disc even while dragging
    const nearestIdx = this.#findNearestVertexIndex();
    const itemIndex = nearestIdx % Math.max(1, this.items.length);
    if (itemIndex !== this.currentItemIndex) {
      this.currentItemIndex = itemIndex;
      this.onActiveItemChange(itemIndex);
    }
    if (this.onPositionChange) {
      const worldPos = this.#getVertexWorldPosition(nearestIdx);
      this.onPositionChange(this.#projectToScreen(worldPos));
    }
  ...
  #projectToScreen(worldPos) {
    ...
    const ndc = vec3.transformMat4(vec3.create(), worldPos, vp);
    const x = (ndc[0] + 1) / 2 * this.viewportSize[0];
    const y = (1 - ndc[1]) / 2 * this.viewportSize[1];
    return { x, y };
  }
}
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <h2 ref={titleRef} className="face-title" />
