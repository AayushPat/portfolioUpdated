...
const Waves = ({
  ...
  useEffect(() => {
    ...
    function updateMouse(x, y) {
        const mouse = mouseRef.current;
        const b = containerRef.current.getBoundingClientRect(); // always live!
        mouse.x = x - b.left;
        mouse.y = y - b.top;
        if (!mouse.set) {
  ...
  return (
    <div
      ...
      className={`absolute z-0 left-0 w-full h-[105hv]] ${className}`}
    >
      <div
      />
      <canvas
        ref={canvasRef}
        className="block w-full h-[200vh] z-1"
      />
    </div>
  );
}
